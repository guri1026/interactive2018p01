...
void ofApp::setup(){
    ...
    serial.setup("/dev/cu.usbserial-DM00B6DF", 9600);
    madeConnect = false;
    ...
    gui.add(move_min.setup("move_min", 420, 0, 900));
    gui.add(move_max.setup("move_max", 700, 0, 900));
    gui.add(sensu_lim.setup("open_lim", 30, 20, 40));
...
//--------------------------------------------------------------
void ofApp::draw(){
    ofDrawBitmapString("player01", 220, 50);
...
void ofApp::shake(){
    ofxOscMessage m;
    m.setAddress("/player01/shake/yes");
...
void ofApp::no_shake(){
    ofxOscMessage m;
    m.setAddress("/player01/shake/no");
    sender.sendMessage(m);
}

void ofApp::right_move(){
    ofxOscMessage m;
    m.setAddress("/player01/move/right");
    sender.sendMessage(m);
...
void ofApp::left_move(){
    ofxOscMessage m;
    m.setAddress("/player01/move/left");
    sender.sendMessage(m);
}

void ofApp::straight_move(){
    ofxOscMessage m;
    m.setAddress("/player01/move/straight");
    sender.sendMessage(m);
}

void ofApp::sensu_open(){
    ofxOscMessage m;
    m.setAddress("/player01/sensu/open");
    sender.sendMessage(m);
}

void ofApp::sensu_close(){
    ofxOscMessage m;
    m.setAddress("/player01/sensu/close");
    sender.sendMessage(m);
}
void ofApp::sensu_stand(){
    ofxOscMessage m;
    m.setAddress("/player01/sensu/stand");
    sender.sendMessage(m);
}
